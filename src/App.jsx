...
export default function App() {
  ...
  // Gallery Navigation
  const nextProject = useCallback(() => setCurrentProjectIdx(prev => (prev + 1) % PROJECTS.length), []);
  const prevProject = useCallback(() => setCurrentProjectIdx(prev => (prev - 1 + PROJECTS.length) % PROJECTS.length), []);
      ...
      <section id="services" className="py-24 md:py-40 px-6 max-w-7xl mx-auto relative z-10">
        ...
        <div className="hidden md:grid grid-cols-2 gap-px bg-white/10 border border-white/10 rounded-[3rem] overflow-hidden">
          {SERVICES.map((s) => (
            ...
            >
              <div className="text-pink-500 mb-8 group-hover:scale-110 transition-transform origin-left">
